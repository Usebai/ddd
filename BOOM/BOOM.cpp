			...
			PreSetValueInfo->DataSize == sizeof(BOOM_PROCESS_OPERA))
		{
			...
			case TAG_READ:
			{
				...
				if (NT_SUCCESS(Status) && process != NULL && g_pSRA->pfn_MmIsAddressValid(pInputData->buf))
				{
					...
					//
					/*
					PMDL g_pmdl2 = g_pSRA->pfn_IoAllocateMdl(targetAddress, targetSize, 0, 0, NULL);
					if (g_pmdl2)
					{
						__try
						{
							g_pSRA->pfn_MmProbeAndLockPages(g_pmdl2, KernelMode, IoReadAccess);
						}
						__except (EXCEPTION_EXECUTE_HANDLER)
						{
							;
						}
						g_pSRA->pfn_MmUnlockPages(g_pmdl2);
						g_pSRA->pfn_IoFreeMdl(g_pmdl2);
					}
					*/
	...
	)
{
	...
	g_pSysRotineAddr->pfn_NtQueryVirtualMemory = \
		(fn_NtQueryVirtualMemory)SSDT::GetFunctionAddress("NtQueryVirtualMemory");
	if (g_pSysRotineAddr->pfn_NtQueryVirtualMemory == NULL)
	{
		return status;
	}
	...
	_In_ PUNICODE_STRING  RegistryPath)
{
	...
	{
		KdPrint((" [BOOM] Initialize() failed...\r\n"));
		return STATUS_UNSUCCESSFUL;
	}
	if (!Undocumented::UndocumentedInit())
	{
		KdPrint((" [BOOM] Initialize2() failed...\r\n"));
		return STATUS_UNSUCCESSFUL;
	}
