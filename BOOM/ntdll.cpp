...
NTSTATUS NTDLL::Initialize()
{
    ...
    {
#ifdef _DEBUG
        DbgPrint("[BOOM] KeGetCurrentIrql != PASSIVE_LEVEL!\n");
#endif
    ...
    if(NT_SUCCESS(NtStatus))
    {
        ...
        if(NT_SUCCESS(NtStatus))
        {
            ...
            Log("[BOOM] FileSize of ntdll.dll is %08X!\r\n", StandardInformation.EndOfFile.LowPart);
            ...
            if(!NT_SUCCESS(NtStatus))
            {
                ...
                Log("[BOOM] ZwReadFile failed with status %08X...\r\n", NtStatus);
            }
        }
        else
            Log("[BOOM] ZwQueryInformationFile failed with status %08X...\r\n", NtStatus);
        ZwClose(FileHandle);
    }
    else
        Log("[BOOM] ZwCreateFile failed with status %08X...\r\n", NtStatus);
    return NtStatus;
}
...
int NTDLL::GetExportSsdtIndex(const char* ExportName)
{
    ...
    if(SsdtOffset == -1)
    {
        Log("[BOOM] SSDT Offset for %s not found...\r\n", ExportName);
    }
